...
The [Brain Computer Interface](BCI/BCI-overview) dataset contains recordings from layer 2/3 neurons in mouse primary motor cortex (M1) during an optical brain-computer interface (BCI) learning task. In this paradigm, mice learn to control the position of a motorized reward port by modulating the activity of a single "conditioned neuron" imaged in M1, allowing for precise experimental control over the circuits involved in learning. In addition to the BCI task recordings, targeted 2-photon single-cell photostimulation was performed before and after learning to measure functional connectivity. This dataset is designed to investigate the learning rules that govern synaptic plasticity in cortical circuits during learning. 
