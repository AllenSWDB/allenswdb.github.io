...
| Slc32a1-IRES-Cre | A pan-inhibitory driver - drives expression in GABAergic neurons across the brain. Cre is knocked into the *Slc32a1* locus, which encodes the vesicular GABA transporter (VGAT), using an IRES sequence so that Cre is expressed without disrupting the endogenous transporter. |  | Visual Learning |
| Sst-IRES-Cre | Drives expression in Somatostatin inhibitory interneurons. | | Visual Coding 2P, Visual Behavior 2P, Visual Coding Neuropixels, Visual Behavior Neuropixels, Dynamic Routing|
...
| Vip-IRES-Cre | Drives expression in Vasoactive Intestinal Peptide inhibitory interneurons. | | Visual Coding 2P, Visual Behavior 2P, Visual Coding Neuropixels, Visual Behavior Neuropixels, Dynamic Routing |
...
|  | TetO-GCaMP6s | Tet dependent GCaMP6s indicator expressing GCaMP6 <i>slow</i>. Simpler two-transgene system that does not require Cre. For example, when crossed with Camk2a-tTA, GCaMP6s is expressed broadly in excitatory neurons. |MGI:5553332 | V1 Deep Dive |
...
|  | VGAT-ChR2-EYFP | Direct (non-Cre-dependent) BAC transgenic expressing channelrhodopsin-2 fused to EYFP in GABAergic neurons under the VGAT (Slc32a1) promoter. Cells are depolarized by blue light (~470 nm peak). | MGI:4950553 | Dynamic Routing |
|  | TIT2L-jGCaMP8s-ICL-IRES-tTA2 | Cre/Tet dependent fluorescent jGCaMP8s indicator expressing jGCaMP8 <i>sensitive</i>. Like Ai148 and Ai162, this reporter uses the TIGRE2 construct, with an IRES preceding tTA2 to limit toxicity from transactivator overexpression. jGCaMP8s has a faster rise time and greater sensitivity than the GCaMP6 indicators used in the other datasets, which makes it better suited to resolving the timing of individual events. |  | Visual Learning |
